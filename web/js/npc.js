...
function addNpc(id, settings) {
  ...
  options.animDown = settings.animDown || {
    id: "walk_down",
    speed: 250,
    frames: [
      [0, 0],
  ...
  options.animUp = settings.animUp || {
    id: "walk_up",
    speed: 250,
    frames: [
      [0, 0],
  ...
  options.animRight = settings.animRight || {
    id: "walk_right",
    speed: 250,
    frames: [
      [0, 0],
  ...
  options.animLeft = settings.animLeft || {
    id: "walk_left",
    speed: 250,
    frames: [
      [0, 0],
  ...
  Crafty.c(id, {
    init: function () {
      ...
      this.requires("SpriteAnimation")
        .reel(
          options.animLeft.id,
          options.animLeft.speed,
          options.animLeft.frames
        )
        .reel(
          options.animRight.id,
          options.animRight.speed,
          options.animRight.frames
        )
        .reel(options.animUp.id, options.animUp.speed, options.animUp.frames)
        .reel(
          options.animDown.id,
          options.animDown.speed,
          options.animDown.frames
        )
        //change direction when a direction change event is received
        .bind(id + "Direction", function (direction) {
          if (direction.x < 0) {
          ...
          switch (cmd.cmd) {
            case "gamechar":
              ...
              if (!this.working) {
                ...
                if (!cmd.originalMsg) {
                  cmd.originalMsg =
                    cmd.options[rand(0, cmd.options.length - 1)];
                  if (!cmd.msg) {
                    if (/^\d+$/.test(cmd.originalMsg)) {
                      cmd.msg = alternativeText("¿Qué numero estoy mostrando?");
                    } else {
                ...
                this.bind("listener.result", function (data) {
                  ...
                  if (!foundCommand) {
                    ...
                    if (cmd.help) {
                      ...
                      cmd.msg +=
                        ". Puedes responder: " + formatCommandList(commands);
                    }
                    this.working = false;
                  }
                });
            ...
            case "randy":
              ...
              if (
                ...
                this.trigger(id + "Direction", { x: 0, y: 1 });
                this.y = round(
                  this.y + this.customStep * (eventData.dt / 1000)
                );
              } else if (this.y > this.dx) {
                this.trigger(id + "Direction", { x: 0, y: -1 });
                this.y = round(
                  this.y + this.customStep * -1 * (eventData.dt / 1000)
                );
              } else {
                this.trigger(id + "Direction", { x: 0, y: 0 });
                this.dx = undefined;
  ...
  npc = Crafty.e(
    "2D, DOM, " + options.animDown.id + ", " + id + ", Animate"
...
function spawnFlayingBird(toLeft) {
  ...
  addNpc(
    "npcFlayingBird",
    getBird(rand(5, 8), 1000, rand(10, 600), 150, actions)
  );
}
